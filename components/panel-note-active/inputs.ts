...
import { useComponent, useValueChanged } from '@/utils/react-utils';
...
export const useInputs = () => {
  ...
  const note = useMemo(() => {
    return store.$state.notes.findOrThrow(n => n.id === activeNoteId);
  }, [activeNoteId]);
  useValueChanged(note, note => editor.current!.dispatch({
    changes: {
      from: 0,
