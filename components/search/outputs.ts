...
export const useOutputs = (props: Props, inputs: Inputs) => {
  ...
  return {
    ...
    onAutocompleteSelected: (value: string | null) => {
      inputs.autocompleteText && store.$local.autocompleteText.$set('');
    ...
    onMouseOverSelectedSynonym: (hoveredSynonymId: SynonymId) => {
      store.$local.hoveredSynonymId.$set(hoveredSynonymId);
    },
    onMouseOutSelectedSynonym: () => {
      store.$local.hoveredSynonymId.$set(null);
    },
    ...
    onClickTabButton: () => {
      if (inputs.showingTab === 'search')
        return store.$local.$patch({ showingTab: 'results', showSearchPane: false, showResultsPane: true })
      if (inputs.showingTab === 'results')
        return store.$local.$patch({ showingTab: 'search', showSearchPane: true, showResultsPane: false })
    },
    onClickCloseButton: () => {
      props.onHide();
    },
